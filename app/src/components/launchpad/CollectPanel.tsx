...
import { DEAD, launchpad, type Quote } from "@/lib/launchpad/config";
import { fmtQuote, fmtTokens, pipsToPct } from "@/lib/launchpad/math";
...
export default function CollectPanel({
  ...
  const [phase, setPhase] = useState<{ k: "idle" } | { k: "busy"; what: "collect" | "claim" } | { k: "sent"; hash: Hex; what: "collect" | "claim" } | { k: "error"; message: string }>({ k: "idle" });
  ...
  // One Claim button: the locker claims one currency per transaction, so claim each credited currency in turn.
  async function send(what: "collect" | "claim", currencies: Address[] = [quote.address]) {
    ...
    try {
      ...
      for (const currency of what === "collect" ? [null] : currencies) {
        let hash: Hex;
        if (currency === null) {
          ...
          const { request } = await pub.simulateContract({ address: LOCKER_ADDRESS, abi: LAUNCH_LOCKER_ABI, functionName: "claim", args: [currency], account: address, dataSuffix: BUILDER_DATA_SUFFIX });
          hash = await wallet.writeContract(request);
        }
        setPhase({ k: "sent", hash, what });
      ...
      {lpFee === 0 ? (
        ...
        <>
          ...
          <div className="flex flex-wrap gap-2 pt-1">
            ...
            {claims.length ? (
              <button type="button" onClick={() => void send("claim", claims.map((c) => c.currency))} disabled={busy || !onChain} className={`${btn.secondarySm} flex-1`} title="A payout to you could not be delivered and was credited instead">
                {phase.k === "busy" && phase.what === "claim" ? <><Spinner size={13} /> Claiming…</> : phase.k === "sent" && phase.what === "claim" ? <><Spinner size={13} /> Confirming…</> : `Claim ${claims.map((c) => c.label(c.raw)).join(" + ")}`}
              </button>
            ) : null}
          </div>
